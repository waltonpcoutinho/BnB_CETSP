   ...
   for ( int i = 0; i < sizeInst; i++ ){
      for ( int j = i + 1; j < sizeInst; j++ ){
         for ( int k = j + 1; k < sizeInst; k++ ){				
            ...
            feasibilityTest = BnB->check_feasibility_Q( tempSequence, solutionXYZ );
...
bool BranchNBound::check_feasibility_Q( vector< int > sequence, vector< vector< double > > solution )
{
   vector< double > c( 3 );		
   vector< double > p1( 3 );		
   vector< double > p2( 3 );
   ...
   for ( int i = 0; i < sizeOfInstance; i++ ){
      for ( int j = 0; j < solutionTemp[ 0 ].size() - 1; j++ ){
